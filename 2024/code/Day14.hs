...
main = do
  ...
  putStrLn $ "Part 2: " ++ show
    (snd . maximum . sort . zip (map entropy bs) $ [1 ..])
  putStrLn $ "Part 2: " ++ show
    (snd . minimum $ zip (map approxKolmogorov bs) [1 ..])
  putStrLn $ "Part 2: " ++ show
    (head $ filter (or . fst) (zip (map (map and . divvy 20 1) bs) [1 ..]))
